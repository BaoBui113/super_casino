...
const GuidDeposit = ({ title, subtitle }: { title: string; subtitle?: string }) => {
    return (
        <div className='flex gap-1 items-start'>
            <div className='w-[9px] h-[9px] border border-solid border-[#777777] mt-1' />
            <div className='flex flex-col gap-2'>
                <span className='font-medium text-sm leading-5 text-[#111111]'>{title}</span>
                {subtitle && <span className='font-normal text-xs leading-4 text-[#444444]'>{subtitle}</span>}
...
export default function PaymentForm({ status }: { status: 'deposit' | 'withdraw' }) {
    ...
    const onSubmit = async (data: FormDeposit) => {
        ...
        if (status === 'deposit') {
            await handleDeposit(customData).then((res) => {
                if (res.status === '0') {
            ...
            await handleWithDraw(customData).then((res) => {
                if (res.status === '0') {
                    toast.success(res.message);
                    handleSetValueInit();
                }
                                ...
                                <div className='flex flex-wrap gap-[30px] items-center mt-10'>
                                    {
                                        listAmount.map((amt, inx) => {
                                            return (
                                                <p onClick={() => handleSelectPayment(amt.value)} className='text-white font-normal text-xs leading-4 px-5 py-2 cursor-pointer' key={inx}>{amt.label}</p>
                                            )
                                        })
                                    }
                                </div>
                    ...
                    render={({ field }) => (
                        <>
                            ...
                            <div className='max-w-[300px] flex gap-3 items-end'>
                                ...
                                >
                                    <option value="" disabled>확장게임 선택</option>
                                    {listGames.map((game) => (
            ...
            <div className='mb-7 w-full'>
                ...
                <div className='bg-white py-5 px-1 flex flex-col gap-2'>
                    {list_guides.map((guid, inx) => {
                        return (
                            <GuidDeposit key={inx} title={guid.title} subtitle={guid.subtitle} />
                        )
                    })}
